...
@section('title', 'Career About Page meta')
@section('css')
  <link rel="stylesheet" href="{{ asset('css/amsify.suggestags.css') }}">
@endsection
@section('page-name', 'about_page_meta')
@section('page-header', 'Custom About Page Meta')
@section('content')
          ...
          <div class="card-body">
            <div class="tab-content">
              ...
              <div class="tab-pane text-center" id="tab-meta-thumbnail" role="tabpanel">
                <img src="{{ Storage::url($metaAbout->thumbnail) }}" alt="{{ $metaAbout->title }}" height="200">
...
@section('script')
      ...
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title">Change Thumbnail Page About</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                  </button>
              </div>
              <div class="modal-body">
                <form method="post" action="{{ route('admin.meta-page.updateThumbnail', $metaAbout->id) }}" id="formChangeThumbnail" enctype="multipart/form-data">
                  @csrf @method('PUT')
                  <input type="hidden" name="page_name" value="about_page">
                  <label for="file-img-favicon" class="upload_image_label">
                    <img src="" height="70" class="thumbnail-preview">
                    <i class='bx bx-upload'></i>
                    <span>Click to upload</span>
                  </label>
                  <input type="file" id="file-img-favicon" class="upload_image" name="thumbnail_about" accept=".webp, image/jpeg, .svg, .gif">
                </form>
              </div>
  ...
  <div class="modal fade" id="addKeyword" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLongTitle">Add new keyword for about page</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                  </button>
              </div>
              <div class="modal-body">
                <form method="post" action="{{ route('admin.meta-page.keyword.store') }}" id="add_keyword">
                  @csrf
                  <input type="hidden" name="meta_page_id" value="{{ $metaAbout->id }}">
    ...
    $(document).ready(function() {
      ...
      $('#tab-meta-description #description-editable').focusout(function(){ //update description
        if (description != $(this).text()) {
          $.ajax({
            type: 'put',
            url:  '{{ route('admin.meta-page.update', $metaAbout->id) }}',
            data: {
